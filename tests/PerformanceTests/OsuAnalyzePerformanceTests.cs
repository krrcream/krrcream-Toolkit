...
namespace krrTools.Tests.PerformanceTests
{
    public class OsuAnalyzePerformanceTests : IDisposable
    {
        ...
        public async Task Analyze_ShouldHandleConcurrentRequests()
        {
            ...
            List<Task<OsuAnalysisResult>> tasks = Enumerable.Range(0, simulatedFileCount)
                                                            .Select(i => Task.Run(() => OsuAnalyzer.AnalyzeAsync(sampleFilePath)))
                                                            .ToList();

            OsuAnalysisResult[] results = await Task.WhenAll(tasks);
            stopwatch.Stop();

            // 验证所有结果
            foreach (OsuAnalysisResult result in results)
            {
