...
namespace krrTools.Tools.N2NC
{
    ...
    public class N2NCViewModel : ToolViewModelBase<N2NCOptions>, IPreviewOptionsProvider
    {
        ...
        public N2NCViewModel(N2NCOptions options)
            : base(ConverterEnum.N2NC, true, options)
        {
            InitializeLocalized();
            ...
            try
            {
                switch (e.PropertyName)
                {
                    case nameof(Options.TargetKeys):
                        ...
                        if (Options.MaxKeys.Value > Options.TargetKeys.Value) Options.MaxKeys.Value = Options.TargetKeys.Value;

                        // 确保MinKeys不超过TargetKeys
                        if (Options.MinKeys.Value > Options.TargetKeys.Value) Options.MinKeys.Value = Options.TargetKeys.Value;
                    ...
                    case nameof(Options.MaxKeys):
                        ...
                        // 确保MaxKeys >= MinKeys
                        if (Options.MaxKeys.Value < Options.MinKeys.Value) Options.MaxKeys.Value = Options.MinKeys.Value;
                        OnPropertyChanged(nameof(MinKeysMaximum)); // 通知计算属性更新
                        break;

                    case nameof(Options.MinKeys):
                        // 确保MinKeys <= MaxKeys
                        if (Options.MinKeys.Value > Options.MaxKeys.Value) Options.MinKeys.Value = Options.MaxKeys.Value;
                        OnPropertyChanged(nameof(MinKeysMaximum)); // 通知计算属性更新
                        break;
        ...
        public double MinKeysMaximum
        {
            get => MaxKeys;
        }
        public double MaxKeysMaximum
        {
            get => TargetKeys;
        }
        ...
        public string MaxKeysDisplay
        {
            get => string.Format(Strings.N2NCMaxKeysTemplate.GetLocalizedString().Value, MaxKeys);
        }
        public string MinKeysDisplay
        {
            get => string.Format(Strings.N2NCMinKeysTemplate.GetLocalizedString().Value, MinKeys);
        }
