...
namespace krrTools.Tests.Beatmaps
{
    public class OldSRCalculator
    {
        ...
        public double Calculate(List<Note> noteSequence, int keyCount, double od, out Dictionary<string, long> times)
        {
            ...
            List<List<Note>> LNSeqByColumn = LNDict.Values.OrderBy(list => list[0].K).ToList();
        ...
        private double[] CalculateSection24()
        {
            ...
            {
                XKs[k] = new double[T];
                Note[] notesInPair = k == 0 ? noteSeqByColumn[0] :
        ...
        private (double[] Rbar, double[] Is) CalculateSection27()
        {
            ...
            for (int i = 0; i < tailSeq.Length; i++)
            {
                ...
                Note nextNote = FindNextNoteInColumn(tailSeq[i], noteSeqByColumn[k]);
                (_, int h_j, int t_j) = (nextNote.K, nextNote.H, nextNote.T);
