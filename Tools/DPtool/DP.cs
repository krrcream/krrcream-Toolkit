...
public class DP : IApplyToBeatmap
{
    ...
    private static int[,] ProcessMatrixStatic(int[,] orgMTXData, DPToolOptions options)
    {
        ...
        if (options.LRemove) RemoveHalf(orgL, true); // Remove left half

        if (options.RRemove) RemoveHalf(orgR, false); // Remove right half
    ...
    private static void RemoveHalf(int[,] matrix, bool isLeft)
    {
        ...
        for (var j = 0; j < cols; j++)
            if (isLeft && j < half)
                matrix[i, j] = -1; // 标记为删除
            else if (!isLeft && j >= half) matrix[i, j] = -1; // 标记为删除
    }
