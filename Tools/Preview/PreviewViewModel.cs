...
namespace krrTools.Tools.Preview
{
    ...
    public class PreviewViewModel : ReactiveViewModelBase
    {
        ...
        // 响应式属性
        private readonly Bindable<FrameworkElement?> _originalVisual = new Bindable<FrameworkElement?>();
        private readonly Bindable<FrameworkElement?> _convertedVisual = new Bindable<FrameworkElement?>();
        private readonly Bindable<string> _title = new Bindable<string>(string.Empty);
        ...
        public PreviewViewModel()
        {
            ...
            IDisposable settingsSubscription = EventBus.Subscribe<SettingsChangedEvent>(OnSettingsChanged);
            Disposables.Add(settingsSubscription);

            IDisposable beatmapChangedSubscription = EventBus.Subscribe<BeatmapChangedEvent>(OnBeatmapChanged);
            Disposables.Add(beatmapChangedSubscription);

            IDisposable refreshSubscription = EventBus.Subscribe<ConvPrevRefreshOnlyEvent>(OnPreviewRefreshRequested);
        ...
        private bool ShouldTriggerRefresh(SettingsChangedEvent settingsEvent)
        {
            ...
            PropertyInfo? property = settingsEvent.SettingsType.GetProperty(settingsEvent.PropertyName);
        ...
        {
            IPreviewProcessor? oldProcessor = Processor;
            Processor = processor;
