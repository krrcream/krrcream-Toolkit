...
namespace krrTools.Tools.N2NC
{
    ...
    public class N2NC
    {
        ...
        {
            float originalCS = beatmap.DifficultySection.CircleSize;
            //修改CS
            ...
            string[] requiredTags = new[] { Strings.ConverterTag, Strings.NToNCTag, "Krr" };
        ...
        public void TransformBeatmap(Beatmap beatmap, N2NCOptions options)
        {
            ...
            Matrix processedMatrix = ProcessMatrix(matrix, timeAxis, beatmap, options, random);
                                     ...
                                     Random random)
        {
            ...
            int maxKeys = (int)options.MaxKeys.Value;
            int minKeys = (int)options.MinKeys.Value;
            double convertTime = Math.Max(1, (TransformSpeedValues[(int)options.TransformSpeed.Value] * 60000 / beatmap.MainBPM * 4) - 10);
            // 使用传入的随机数生成器
            Random RG = random;
        ...
        /// 封装成其他文件中也能使用的方法
        /// </summary>
        public Matrix DoKeys(Matrix matrix,
                             ...
                             Random random)
        {
            ...
            if (turn >= 0)
            {
                ...
                (Matrix oldMTX, Matrix insertMTX) = convertMTX(turn, timeAxis, convertTime, CS, random, maxKeysequal);
            ...
            {
                // RemoveKeys 逻辑
                Matrix newMatrix = SmartReduceColumns(matrix, timeAxis, -turn, convertTime, beatlengthAxis, random);
                DensityReducer(newMatrix, maxKeys, minKeys, targetKeys, random);
                return newMatrix;
            }
        }
        ...
        public Span<int> GenerateEndTimeIndex(List<ManiaNote> maniaObjects)
        {
            ...
            foreach (ManiaNote item in maniaObjects) result.Add(item.EndTime);
        ...
        public (Matrix, Matrix) convertMTX(int turn,
                                           Span<int> timeAxis,
                                           double convertTime,
                                           int CS,
                                           Random random,
        ...
        // 转换操作
        public Matrix convert(Matrix matrix,
                              Span<int> EndTimeIndexAxis,
                              Matrix oldMTX,
                              Matrix insertMTX,
                              Span<int> orgColIndex,
                              ...
                              bool maxKeysEqualTargetKeys)
        {
            try
            {
                ...
                int turn = oldMTX.Cols; // oldMTX的列数
                // 1.初步convert。MappingStep1
                Matrix newMatrix = PerformInitialConvert(matrix, oldMTX, insertMTX, targetKeys, turn, rows, originalCols, maxKeysEqualTargetKeys);
                // 2.生成位置映射
        ...
        /// 执行初步转换 - MappingStep1
        /// </summary>
        private Matrix PerformInitialConvert(Matrix matrix, Matrix oldMTX, Matrix insertMTX, int targetKeys, int turn, int rows, int originalCols, bool ifMaxKeysequal)
        {
            ...
            for (int i = 0; i < rows; i++)
            {
                ...
                // 初始化为-1
                for (int k = 0; k < targetKeys; k++) tempRow[k] = -1;
        ...
        /// 生成位置映射
        /// </summary>
        private BoolMatrix GenerateDeleteMark(Matrix newMatrix, Span<int> timeAxis, Span<int> EndTimeIndexAxis, Span<double> beatLengthAxis, Span<int> orgColIndexAxis, int targetKeys)
        {
            ...
            for (int i = targetKeys; i < newMatrixSpan.Length; i++)
            {
                ...
                //处理面尾（注意是索引轴）
                if (preOldIndex >= 0) endTimeTempRow[col] = Math.Max(EndTimeIndexAxis[preOldIndex], endTimeTempRow[col]);

                if (timeAxis[row] < endTimeTempRow[col] + space - 10) markSpan[i] = true;
                ...
                if (timeAxis[row] < convertTimePointRow[col] + space + 10) markSpan[i] = true;
            }
        ...
        private void ShiftInsert<T>(T nums, int insertIndex) where T : IList<int>
        {
            ...
            {
                // 从右向左移动元素，避免覆盖
                for (int i = nums.Count - 1; i > insertIndex; i--) nums[i] = nums[i - 1];
        ...
        public Matrix SmartReduceColumns(Matrix orgMTX,
                                         Span<int> timeAxis,
                                         int turn,
                                         double convertTime,
                                         Span<double> beatLengthAxis,
        ...
        private void ProcessRegion(Matrix orgMTX,
                                   Matrix newMatrix,
                                   Matrix originColumnMap,
                                   Span<int> timeAxis,
                                   int regionStart,
                                   int regionEnd,
                                   int targetCols,
                                   ...
                                   Random random)
        {
            ...
            // 处理长条延续部分
            for (int row = regionStart; row <= regionEnd; row++) HandleLongNoteExtensions(newMatrix, row, targetCols);
        ...
        private void ApplyMinimumNotesConstraint(Matrix matrix,
                                                 Matrix orgMTX,
                                                 int startRow,
                                                 int endRow,
                                                 int targetCols,
        ...
        private List<int> GetColumnsToRemove(int[] columnWeights,
                                             int targetCols,
                                             int originalCols,
                                             Matrix orgMTX,
                                             int regionStart,
        ...
        private double CalculateColumnRisk(Matrix matrix,
                                           int colIndex,
                                           int totalCols,
                                           int regionStart,
                                           int regionEnd)
        ...
        private void CopyLongNoteBody(Matrix orgMTX,
                                      Matrix newMatrix,
                                      int startRow,
                                      int oldCol,
                                      int newCol,
        ...
        private void ProcessEmptyRows(Matrix orgMTX,
                                      Matrix newMatrix,
                                      Span<int> timeAxis,
                                      Span<double> beatLengthAxis,
                                      Random random)
        ...
        // 尝试直接插入note到当前行的可用位置
        private bool TryInsertNoteDirectly(Matrix newMatrix,
                                           Matrix orgMTX,
                                           Span<int> timeAxis,
                                           int row,
                                           int targetCols,
        ...
        // 检查长条尾部是否过于接近下一个note
        private bool IsHoldNoteTailTooClose(Matrix newMatrix,
                                            Matrix orgMTX,
                                            Span<int> timeAxis,
                                            int row,
                                            int orgCol,
                                            ...
                                            double beatLength)
        {
            double minTimeDistance = (beatLength / 2.5) - 10; // 注意这里是-10
        ...
        private void TryClearSpaceAndInsert(Matrix orgMTX,
                                            Matrix newMatrix,
                                            Span<int> timeAxis,
                                            int emptyRow,
                                            int targetCols,
        ...
        private bool IsPositionAvailableForEmptyRow(Matrix matrix,
                                                    Span<int> timeAxis,
                                                    int row,
                                                    int col,
                                                    double beatLength)
