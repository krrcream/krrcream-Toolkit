    ...
    public partial class KRRLVViewModel : ObservableObject, INotifyPropertyChanged
{
    ...
    public event PropertyChangedEventHandler PropertyChanged;    public KRRLVViewModel()
    {
    ...
    private void Browse()
    {
        ...
        if (dialog.ShowDialog() == true)
        {
            ...
            ProcessDroppedFiles(new string[] { dialog.FileName });
        }
    }
    ...
    private void Save()
    {
        ...
        if (dialog.ShowDialog() == true)
        {
            try
            {
                ...
                // 添加CSV头部
                csv.AppendLine("KRRLV,XXYSR,Title,Diff,Artist,Creator,Keys,BPM,OD,HP,LN%,beatmapID,beatmapsetId,filePath");
    ...
    public async void ProcessDroppedFiles(string[] files)
{
    ...
    private void ProcessOszEntry(ZipArchiveEntry entry, string oszFilePath)
    {
        try
        {
            ...
            // 检查是否已存在于列表中
            if (OsuFiles.Any(f => f.FilePath.Equals(uniqueId, StringComparison.OrdinalIgnoreCase)))
                return;
    ...
    private void AnalyzeOszEntry(OsuFileItem item, ZipArchiveEntry entry)
{
        ...
        using (var memoryStream = new MemoryStream())
        {
            ...
            try
            {
                ...
                Application.Current.Dispatcher.Invoke(() =>
                {
                    ...
                    item.XXYSR = result.XXYSR;
                    item.KRRLV = result.KRRLV;
                    item.Status = "已分析";
                });
            }
            finally
    ...
    {
        // 检查文件是否已存在于列表中
        if (OsuFiles.Any(f => f.FilePath.Equals(filePath, StringComparison.OrdinalIgnoreCase)))
            return;
